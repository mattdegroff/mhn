...
cd /tmp
wget https://github.com/RedolentSun/hpfeeds/releases/download/libev-4.15/libev-4.15.tar.gz
...
cd /opt
rm -rf /opt/hpfeeds
git clone https://github.com/RedolentSun/hpfeeds
chmod 755 -R hpfeeds
cd hpfeeds
...
pip install pymongo
pip install -e git+https://github.com/RedolentSun/evnet.git#egg=evnet-dev
pip install .
deactivate
